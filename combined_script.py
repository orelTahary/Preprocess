...
# %%
timeBegin = time.time()
pp.bandpass_filter(os.path.join(dataDir, 'binNew'), os.path.join(dataDir, 'binBand'), 'Elec{0}' + rangeStr + '.bin',
                   elecList, freq=[300, 6000])
timeEnd = time.time()
...
timeBegin = time.time()
pp.remMedian(os.path.join(dataDir, 'binNew'), os.path.join(dataDir, 'binMedRaw'), elecList, rangeStr, batchSize=1000000,
             verbose=False)
timeEnd = time.time()
...
timeBegin = time.time()
pp.bandpass_filter(os.path.join(dataDir, 'binMedRaw'), os.path.join(dataDir, 'binMed'), 'Elec{0}' + rangeStr + '.bin',
                   elecList, freq=[300, 6000])
timeEnd = time.time()
print(f"Filtered {len(elecList)} electrodes in {timeEnd - timeBegin:.2f} seconds")
